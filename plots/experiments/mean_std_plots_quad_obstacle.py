...
COLLISIONS_OBST_SCALE = (
    (TIME_METRIC_COLLISION / EPISODE_DURATION) / NUM_AGENTS
)  # Not times two because 1 collision = 1 drone collide with 1 obstacle, and we only talk about drones here
...
PLOTS = [
    ...
    dict(key='0_aux/avg_rewraw_crash',
    ...
    dict(key='0_aux/avg_num_collisions_after_settle',
         ...
         clip_min=0.1),
    dict(key='0_aux/avg_num_collisions_obst_quad',
...
def main():
    ...
    plt.savefig(os.path.join(os.getcwd(), f'../final_plots/quads_obstacle_electron.pdf'),
                format='pdf',
                bbox_inches='tight',
                pad_inches=0.01)
