...
def get_action_distribution(action_space, raw_logits, num_options=None):
    """
    ...
    if num_options is None:
        ...
        assert num_options == raw_logits.shape[-1]
        assert calc_num_logits(action_space) == raw_logits.shape[-2]
...
class CategoricalOptionDistribution:
    def __init__(self, raw_logits, num_options):
        """
        Ctor.
        :param raw_logits: unprocessed logits, typically an output of a fully-connected layer
        :param num_option: number of options
        """

        self.raw_logits = raw_logits
        self.num_batch = self.raw_logits.shape[0]
        self.num_actions = self.raw_logits.shape[-2]
