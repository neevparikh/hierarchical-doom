...
class ReinforcementLearningAlgorithm(AlgorithmBase, ABC):
    ...
    def add_cli_args(cls, parser):
        ...
        p.add_argument('--keep_checkpoints',
                       default=3,
                       type=int,
        ...
        p.add_argument(
            ...
            type=int,
            help=
            'Save intermediate checkpoints in a separate folder for later evaluation (default=never)'
        )

        p.add_argument('--stats_avg',
                       default=100,
                       type=int,
        ...
        p.add_argument('--train_for_env_steps',
                       default=int(1e10),
                       type=int,
                       help='Stop after all policies are trained for this many env steps')
        p.add_argument('--train_for_seconds',
                       default=int(1e10),
                       type=int,
        ...
        p.add_argument(
            ...
            type=float,
            help=
            'Observation preprocessing, mean value to subtract from observation (e.g. 128.0 for 8-bit RGB)'
        )
        p.add_argument(
            '--obs_scale',
            default=1.0,
            type=float,
            help=
            'Observation preprocessing, divide observation tensors by this scalar (e.g. 128.0 for 8-bit RGB)'
        )
        ...
        p.add_argument(
            ...
            type=float,
            help=
            'Clip rewards between [-c, c]. Default [-10, 10] virtually means no clipping for most envs'
        )
        ...
        p.add_argument(
            ...
            type=int,
            help=
            'Number of fully-connected layers of size "hidden size" to add after the basic encoder (e.g. convolutional)'
        )
        p.add_argument(
            '--hidden_size',
            default=512,
            type=int,
            help=
            'Size of hidden layer in the model, or the size of RNN hidden state in recurrent model (e.g. GRU)'
        )
        ...
        p.add_argument(
            ...
            type=str2bool,
            help=
            'Use spectral normalization to smoothen the gradients and stabilize training. Only supports fully connected layers'
        )
        ...
        p.add_argument(
            ...
            type=str2bool,
            help=
            'Only for continuous action distributions, whether stddev is state-dependent or just a single learned parameter'
        )
        p.add_argument(
            '--initial_stddev',
            default=1.0,
            type=float,
            help=
            'Initial value for non-adaptive stddev. Only makes sense for continuous action spaces')
